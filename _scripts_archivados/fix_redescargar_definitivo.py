...
# CORRECCION CLAVE: combinar local Y visitante para calcular la liga principal
df_local = df[["equipo_local", "liga"]].rename(columns={"equipo_local": "equipo"})
df_visitante = df[["equipo_visitante", "liga"]].rename(columns={"equipo_visitante": "equipo"})
df_todos = pd.concat([df_local, df_visitante], ignore_index=True)
principales = df_todos.groupby("equipo")["liga"].agg(lambda x: x.value_counts().index[0])
...
for archivo in os.listdir(CACHE_DIR):
    ...
    try:
        ...
        if len(data.get("jugadores", [])) == 0:
            archivos_vacios.append(archivo.replace(".json", ""))
    except Exception:
        pass
...
for equipo in archivos_vacios:
    ...
    try:
        resp_team = requests.get(
            ...
            headers=headers, params={"search": busqueda}, timeout=20
        )
...
print(f"\nOK: {procesados} | sin_liga_csv: {sin_liga_csv} | sin_team: {sin_team} | sin_liga_pais: {sin_liga_pais} | errores: {errores}")
